...
public class CCLevel3 : MonoBehaviour {
	...
	public void canvas11Next (){
		...
		if (System.IO.File.Exists (@"E:\SEDesinoUserData\"+playerName+"CC.txt")) {
			lines = System.IO.File.ReadAllLines (@"E:\SEDesinoUserData\"+playerName+"CC.txt");
			foreach (char c in lines[0])
				level += c.ToString ();
			levelNumber = int.Parse (level);
			if(levelNumber<3)
				System.IO.File.WriteAllText(@"E:\SEDesinoUserData\"+playerName+"CC.txt","3" );
		}
		else{
			//System.IO.File.Create(@"E:\SEDesinoUserData\CC.txt");
			System.IO.File.Create(@"E:\SEDesinoUserData\"+playerName+"CC.txt").Close();
			System.IO.File.WriteAllText(@"E:\SEDesinoUserData\"+playerName+"CC.txt","3" );
